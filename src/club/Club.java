...
public class Club {
    ...
    public boolean sePuedeEliminarSocio(String cedula) {
        try {
            ...
            if (socio.darTipo() == Socio.Tipo.VIP)
                throw new Exception("No se puede eliminar socio VIP");

            if (!socio.darFacturas().isEmpty())
                throw new Exception("Tiene facturas pendientes");

            if (socio.darAutorizados().size() > 1)
                throw new Exception("Tiene más de un autorizado");
